...
function Updatealllogins() {
    ...
    const fetchLoginData = async () => {
      try {
        const response = await axios.get(
          `http://localhost:4000/alllogindetails/${loginId}`
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(
        `http://localhost:4000/alllogindetails/alllogins/${loginId}`,
        data
      );
      navigate("/logins/alllogins");
    } catch (error) {
...
export default Updatealllogins;
